# best to train adapter on Bloom-3B (bigscience/bloom-3b) with kaggle backend
...
def check(args):
    ...
    assert args.source_max_len == SOURCE_MAX_LENGTH
    assert args.target_max_len == TARGET_MAX_LENGTH
...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="bigscience/bloom-3b")
...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    lora_r: int = field(default=LORA_R, metadata={"help": "Lora R dimension."})
    lora_alpha: float = field(default=LORA_ALPHA, metadata={"help": " Lora alpha."})
    lora_dropout: float = field(
        default=LORA_DROPOUT, metadata={"help": "Lora dropout."}
    )
    max_memory_MB: int = field(default=16000, metadata={"help": "Free memory per gpu."})
    report_to: str = field(
        default="wandb",
        metadata={"help": "To use wandb or something else for reporting."},
    )
    output_dir: str = field(
        default=OUTPUT_DIR, metadata={"help": "The output dir for logs and checkpoints"}
    )
    optim: str = field(
    ...
    learning_rate: float = field(
        default=LEARNING_RATE, metadata={"help": "The learning rate"}
    )
    ...
    warmup_ratio: float = field(
        default=WARMUP_RATIO, metadata={"help": "Fraction of steps to do a warmup for"}
    )
    ...
    save_steps: int = field(
        default=SAVE_STEPS, metadata={"help": "How often to save a model"}
    )
    save_total_limit: int = field(
        default=SAVE_TOTAL_LIMIT,
        metadata={
